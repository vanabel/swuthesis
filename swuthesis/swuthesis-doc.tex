...
%
% Copyright (C) 2015-2023 by SWU TeX Users Group <https://github.com/swutug>
%
...
本模板 \cls{swuthesis} 是西南大学本科生和研究生学位论文的 \LaTeX{}
模板， 按照《\href{}
{西南大学研究生学位论文撰写手册}》和
《\href{}
{西南大学本科毕业论文（设计）格式}》的要求编写。
...
下载地址：
\begin{itemize}
  \item 主要地址：\url{https://github.com/swutug/swuthesis/releases}
  \item 学校镜像：\url{https://git.lug.swu.edu.cn/swutug/swuthesis}
  \item 研究生院网站（版本较旧）：
    \url{https://gradschool.swu.edu.cn/column/65}
\end{itemize}

用户在使用 \pkg{swuthesis} 模板前，应先阅读学校的《撰写手册》等规范。
如果在使用的过程中遇到问题，可以阅读
\href{https://github.com/swutug/swuthesis/wiki}{常见问题}，
或者在 \href{https://github.com/swutug/swuthesis/issues}{GitHub Issues}
中反馈。
...
\begin{table}[htb]
  ...
  \begin{tabular}{lll}
    ...
    类别     & 文件                      & 说明                         \\
    \midrule
    模板文件 & \file{swuthesis.cls}     & 模板文件                     \\
             & \file{swuthesis-*.bst}   & \BibTeX{} 参考文献表格式     \\
             & \file{swuthesis-*.bbx}   & BibLaTeX 参考文献表格式      \\
             & \file{swuthesis-*.cbx}   & BibLaTeX 参考文献表格式      \\
             & \file{figures/swu-*.pdf} & 校名和校徽图片               \\
    \midrule
    使用说明 & \file{swuthesis-doc.tex} & 模板使用说明的源代码         \\
             & \file{swuthesis-doc.pdf} & （你正在阅读的）模板使用说明 \\
    \midrule
    示例文档 & \file{main.tex}           & 主文档                       \\
             & \file{swusetup.tex}      & 配置文件                     \\
             & \file{chapters/*.tex}     & 示例文档的各个章节           \\
             & \file{figures/}           & 放置图片的目录               \\
             & \file{bib/swu.bib}       & \BibTeX{} 示例数据库         \\
    \midrule
...
编译说明文档：
\begin{shell}
  latexmk -xelatex swuthesis-doc.tex
\end{shell}
...
\end{itemize}
\begin{latex}
  \documentclass[output=electronic]{swuthesis}
\end{latex}
...
\end{itemize}
\begin{latex}
  \documentclass[section-style=arabic]{swuthesis}
\end{latex}
...
\begin{latex}
  \documentclass[fontset=windows]{swuthesis}
\end{latex}
...
封面由 \cs{maketitle} 命令生成，其中的各项信息使用 \cs{swusetup} 命令以
key-value 的方式填写，如：
\begin{latex}
  \swusetup{
    title  = {论文中文题目},
...
有几点说明：
\begin{itemize}
  \item \cs{swusetup} 使用 \pkg{kvsetkeys} 机制，配置项之间不能有空行，否则会报错。
...
有的院系额外要求声明页有“评审专家签名”，可以在模板中使用 \cs{swusetup} 命令设置
\opt{reviewer = true}。
...
对于特殊的章节，\cls{swuthesis} 还提供了相应的环境：
\begin{itemize}
...
\DescribeOption{keywords*}
摘要的关键词应使用 \cs{swusetup} 的接口进行设置，在录入时使用西文逗号隔开，
比如：
\begin{latex}
  \begin{abstract}
    这里是摘要。
...
\begin{latex}
  ...
  \bibliographystyle{swuthesis-numeric}
\end{latex}
或者
\begin{latex}
  % 著者-出版年制
...
\begin{latex}
  \usepackage[bibstyle=swuthesis-numeric,
              citestyle=swuthesis-inline]{biblatex}
\end{latex}
